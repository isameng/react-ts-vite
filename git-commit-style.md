...
- docs: 文档修改
- style: 代码格式修改, 注意不是 css 修改（例如空格、分号等格式）
- test: 测试用例新增、修改
- build: 影响项目构建或依赖项修改（例如scopes: webpack、gulp、npm等）
- revert: 代码回退，恢复上一次提交
- ci: 持续集成相关文件修改（例如scopes: Travis，Circle）
- chore: 其他修改（不在上述类型中的修改，如改变构建流程、或者增加依赖库、工具等）
